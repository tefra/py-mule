...
class SeeyaSearchRequestMapperTestCase(TestCase):
    ...
    def test_map_route(self):
        actual = self.mapper.map_route(
            RouteRequest(
                departure="ATH", arrival="SKG", datetime="2018-12-31T12:12:06"
            )
        )
    ...
    def test_map_passengers(self, map_passenger):
        ...
        self.assertEqual(
            ["a", "b", "c"], self.mapper.map_passengers([1, 2, 3])
        )

    def test_map_passenger(self):
        actual = self.mapper.map_passenger(
            PassengerRequest(count=2, type=PassengerType.CHD)
        )
...
class SearchResponseMapperTestCase(TestCase):
    ...
    def test_map_recommendation(self, map_legs, map_passengers, id, resources):
        ...
        prices = {SeeyaPassengerType.adults: FooPrice(2)}

        actual = self.mapper.map_recommendation(
            SeeyaRecommendation(
                gds="kiwi",
                deepLink="google.com",
                segRefs=["a", "b"],
        ...
        expected = {
            ...
            "provider": {
                "name": "kiwi",
                "presentationName": "kiwi",
                "uri": "google.com",
            },
            "resources": "$resources",
        }
        self.assertEqual(expected, actual.to_dict())
        map_legs.assert_called_once_with(["a", "b"], data=2)
        map_passengers.assert_called_once_with(prices)

    @patch.object(SearchResponseMapper, "map_segment")
    def test_map_segments(self, map_segment):
        map_segment.side_effect = lambda x: x + 1
        actual = self.mapper.map_segments({"a": 1, "b": 2, "c": 3})
        self.assertEqual({"a": 2, "b": 3, "c": 4}, actual)
    ...
    def test_map_segment(self, map_point, map_stops):
        ...
        actual = self.mapper.map_segment(
            SeeyaSegment(
                dep="fooDep",
                arr="fooArr",
                marketingCarrier="AB",
    ...
    def test_map_point(self):
        actual = self.mapper.map_point(
            SeeyaSegmentPoint(airport="ATH", datetime="2018-12-12 15:30:00")
        )
    ...
    def test_map_technical_stops(self, map_technical_stop):
    ...
    def test_map_technical_stop(self):
        actual = self.mapper.map_technical_stop(
            SeeyaTechnicalStop(
                destination="ATH",
                duration=15,
    ...
    def test_map_legs(self):
        @attrs(these={"cabin": attrib(), "bookingClass": attrib()})
        ...
        self.mapper.segments = dict(
            aaa=FooSegment(), bbb=FooSegment(), ccc=FooSegment()
        )

        actual = self.mapper.map_legs(
            value=SeeyaSegmentReferences.deserialize(
            ...
            "segments": [
                {
                    "bookingClass": None,
                    "cabinClass": {
                        "services": {"bookingClass": "A!", "cabinClass": "A"}
                    },
                }
            ],
        }
            ...
            "segments": [
                {
                    "bookingClass": None,
                    "cabinClass": {
                        "services": {"bookingClass": "B!", "cabinClass": "B"}
                    },
                },
                {
                    "bookingClass": None,
                    "cabinClass": {
                        "services": {"bookingClass": "C!", "cabinClass": "C"}
                    },
                },
            ],
        }
